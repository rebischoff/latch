...
When `bulkUpdate` patch touches verification Fields, create **one pending row per id** with shared **`batch_id`**; live non-gated keys in patch still follow bulk rules per [bulk-operations.md](../../../reference/bulk-operations.md).
...
`bulkUpdate(descriptor, store, ctx, ids, patch, opts)` in [`packages/dal/src/bulk.ts`](../../../../packages/dal/src/bulk.ts) takes **no** `pendingStore` today. Thread it through `bulkUpdate` (and the `createSurfaceDal` bulk wiring) the same way single-record `patch` receives it.
...
5. Define the **one-open-`submitted`-per-entity** interaction inside a batch: an id with an existing open pending → report as a per-row skip (`reason: forbidden_row`/`not_found` per [`bulk-operations.md`](../../../reference/bulk-operations.md) semantics), not a whole-batch 409.
