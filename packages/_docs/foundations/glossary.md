...
See [`../reference/access-control.md`](../../policy/docs/access-control.md) and [`../reference/metadata-and-codegen.md`](../../codegen/docs/reference/metadata-and-codegen.md).
...
Removes the row from live tables. The DAL writes an audit entry with `action = delete` and a `before` snapshot. **No** `deleted_at` tombstone columns in v1. **Recovery** is restore-from-audit (privileged replay), not undelete — see [`scope.md`](./scope.md) and [`../reference/audit-and-lifecycle.md`](../../audit/docs/audit-and-lifecycle.md).
...
A DAL method that processes many ids in one request with per-row permission evaluation and partial-success reporting. See [`architecture/bulk-operations.md`](../../dal/docs/bulk-operations.md).
