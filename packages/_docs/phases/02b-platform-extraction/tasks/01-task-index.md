...
After each task's **Verify** passes: check its boxes, set [`../STATUS.md`](../STATUS.md) **Execute now** → next file, move the task to **Recently completed**. When `06` passes, repoint root [`../../../STATUS.md`](../../../../../docs/STATUS.md) active phase back to **02 UI sync** and resume [`../../02-ui-sync/tasks/04-db-schema.md`](../../02-ui-sync/tasks/04-db-schema.md).
