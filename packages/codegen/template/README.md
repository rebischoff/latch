...
3. `npm run db:migrate -w __APP_PACKAGE__` (or `node scripts/db-migrate.mjs --dir=apps/__APP_SLUG__` from repo root)
4. Add `*.surface.yaml` under `modules/`, then `npm run codegen -w __APP_PACKAGE__`
5. `npm run dev -w __APP_PACKAGE__` (port **__APP_PORT__**)

See [scaffold runbook](__MONOREPO_REL__/packages/codegen/docs/scaffold-runbook.md) for DB reset, verify steps, and lifting domain from `fixtures/crm-proof/`.
