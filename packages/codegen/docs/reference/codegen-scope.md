...
>
> **Status:** Decision (2026-06-06). States what **sync** codegen (`npm run codegen`) **owns** (generates), **verifies** (cross-checks), and **must not own**. App scaffolding is [`../tasks/05-scaffold-cli.md`](../tasks/05-scaffold-cli.md) — a different tool.
...
- **YAML ↔ DB:** `codegen --check` cross-checks declared types against Drizzle; CI fails on drift (threat [T11](../../../docs/foundations/threat-model.md)).
