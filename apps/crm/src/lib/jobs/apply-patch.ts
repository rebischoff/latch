...
/** Column-level snapshot for audit `before` / `after` on patch paths. */
...
/** Full delete `before` for restore replay (anchor + CASCADE `assignments`). */
export const jobDeleteAuditSnapshot = (
  row: MemoryJobRecord,
  assignments: MemoryAssignmentRecord[],
): Record<string, unknown> => ({
  ...jobRowAuditSnapshot(row),
