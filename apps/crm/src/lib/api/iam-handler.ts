...
export type IamApiSuccessBody<T> = {
  data: T;
  manifest: Manifest;
};

export type IamApiErrorBody = {
  error: {
    code: string;
    message: string;
    details?: unknown;
...
export const jsonSuccess = <T>(data: T, manifest: Manifest): Response =>
  Response.json({ data, manifest } satisfies IamApiSuccessBody<T>);

export const mapLatchError = (error: unknown): Response => {
  if (isLatchError(error)) {
    const body: IamApiErrorBody = {
      error: {
        code: error.code,
...
export const withIamApiHandler = async (
