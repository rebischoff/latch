...
import { replaceCategorySpecExcludesTx } from "./item-spec-exclude-write";
...
describe("replaceCategorySpecExcludesTx", () => {
  it("replaces exclude rows for a nested category", async () => {
    ...
    expect(queries.some((sql) => sql.includes("DELETE FROM item_spec_exclude"))).toBe(
      true,
    );
    expect(queries.some((sql) => sql.includes("INSERT INTO item_spec_exclude"))).toBe(
      true,
    );
  });
