...
const treeContext = (): ParticipationContext => ({
  ...
  excludesByItem: new Map([["c-1-1-1", new Set([specD])]]),
  categoriesById: new Map(treeRows.map((row) => [row.id, row])),
});

const visibleAt = (categoryId: string, rootItemId = "c-1") => {
  const ctx = treeContext();
  const row = {
    spec_def_id: specD,
    assign_item_id: "c-1-1",
    excluded_here: ctx.excludesByItem.get(categoryId)?.has(specD) ?? false,
  };
  return isSpecVisibleAtItem(
    categoryId,
    rootItemId,
    row,
    ctx.categoriesById,
...
describe("isSpecVisibleAtItem — generic tree", () => {
...
describe("filterSpecsForItemVisibility — Fire Alarm", () => {
  ...
  it("root shows SLC and unassigned defs but hides child-owned Spec B", () => {
    ...
    const filtered = filterSpecsForItemVisibility(
      faRoot,
      faRoot,
      [def(slc, "SLC Protocol"), def(specB, "Spec B")],
      part,
...
describe("filterSpecsForItemVisibility — unassigned namespace", () => {
  const root = "root";
  ...
  it("shows unassigned defs only at scope root", () => {
    ...
    const atRoot = filterSpecsForItemVisibility(
      root,
      root,
    ...
    const atChild = filterSpecsForItemVisibility(
      "child",
      root,
      [def(unassigned, "Draft")],
      part,
