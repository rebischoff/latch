// DO NOT EDIT — generated from employee_detail.surface.yaml

/**
 * MULTI_TABLE_GLUE_SKIPPED — columns span multiple tables (employee, party).
 * Implement projectRow, applyPatch, and the surface descriptor by hand in this module.
 */
export const MULTI_TABLE_GLUE_SKIPPED = true as const;

export const MULTI_TABLE_GLUE_TABLES = ["employee", "party"] as const;

export const MULTI_TABLE_GLUE_ANCHOR = "employee" as const;
