...
const baseHandlers = createSurfaceRouteHandlers({
  resolveContext,
  resolveContextFresh,
  toDetailInput: (id) => ({ surfaceId: "employee_detail", entityId: id }),
  dal: {
    get: async (ctx, id) => {
      const dal = await getSurfaceDetailDal("employee_detail");
...
export const GET = async (
  ...
  withApiHandler(async () => {
    ...
    const { data, manifest } = await loadSurfaceDetailQuery("employee_detail", id);
...
export const DELETE = async (
  ...
  withSubhubApiHandler(async () => {
    ...
    await dal.employeeDetail.delete(ctx, id);
