...
const baseHandlers = createSurfaceRouteHandlers({
  resolveContext,
  resolveContextFresh,
  toDetailInput: (id) => ({ surfaceId: "item_detail", entityId: id }),
  dal: {
    get: async (ctx, id) => {
      const dal = await getSurfaceDetailDal("item_detail");
...
export const GET = async (
  ...
  withApiHandler(async () => {
    ...
    const { data, manifest } = await loadSurfaceDetailQuery("item_detail", id);
