...
const baseHandlers = createSurfaceRouteHandlers({
  resolveContext,
  resolveContextFresh,
  toDetailInput: (id) => ({ surfaceId: "category_detail", entityId: id }),
  dal: {
    get: async (ctx, id) => {
      const dal = await getSurfaceDetailDal("category_detail");
...
export const GET = async (
  ...
  withApiHandler(async () => {
    ...
    const { data, manifest } = await loadSurfaceDetailQuery("category_detail", id);
