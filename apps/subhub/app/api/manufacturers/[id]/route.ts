...
const baseHandlers = createSurfaceRouteHandlers({
  resolveContext,
  resolveContextFresh,
  toDetailInput: (id) => ({ surfaceId: "manufacturer_detail", entityId: id }),
  dal: {
    get: async (ctx, id) => {
      const dal = await getSurfaceDetailDal("manufacturer_detail");
...
export const DELETE = async (
  ...
  withSubhubApiHandler(async () => {
    ...
    const dal = await ensureContactsDal();
    await dal.manufacturerDetail.delete(ctx, id);
