...
const baseHandlers = createSurfaceRouteHandlers({
  resolveContext,
  resolveContextFresh,
  toDetailInput: (id) => ({ surfaceId: "purchase_order_detail", entityId: id }),
  dal: {
    get: async (ctx, id) => {
      const dal = await getSurfaceDetailDal("purchase_order_detail");
...
export const GET = async (
  ...
  withApiHandler(async () => {
    ...
    const { data, manifest } = await loadSurfaceDetailQuery(
