...
> **Wave:** 1 + **2b scopes & zones** · **Status:** target spec (2026-06-19); **scopes UI** task [37c](../tasks/37c-site-scopes-zones.md) (2026-07-01); **zone tree unification** [42a](../tasks/42a-site-zone-tree-unification.md) (2026-07-14) · **Contrast:** anchor entity (not party lens) — portfolio FKs + standing contacts; **scopes & zones** on Scopes & zones tab — [`customer.md`](./customer.md), [`property-owner.md`](./property-owner.md) · **Catalog:** [`surfaces.md`](../surfaces.md#site_list--site_detail) · **DBML:** `site`, `site_contact`, `site_contact_relation`, `site_zone` · **Decisions:** [site zone tree unification](../decisions/site.md#decision-site-zone-tree-unification-2026-07-14), [site scopes & zones](../decisions/site.md#decision-site-scopes--zones--category-root-instances-2026-06-30), [portfolio FKs](../decisions/site.md#decision-portfolio-fks-on-site_detail--writable-scalars-2026-06-19) · **Planning:** [`11-categories-scope-model.md`](../planning/11-categories-scope-model.md)

**Related:** Hub create via **`add_site`** on [`customer_detail`](./customer.md) and [`property_owner_detail`](./property-owner.md). Relation catalog: [`site-contact-relation.md`](./site-contact-relation.md) ✅. Legacy [`site-geography.md`](./site-geography.md) (`sections`/`locations`) **superseded** by this spec § `scopes` (General bucket retired in 37f / 039).
...
| Anchor table | `site` |
| All tables (DAL) | `site`, `site_contact`, `site_contact_relation` (join for relation labels); read joins `party` for portfolio display names; **`site_zone`** (unified tree, task 42a) |
...
| `scopes` | collection | read + write | `site_zone` roots + nested children | Root = `parent_zone_id` null + `root_item_id`; nested `zones[]` by `parent_zone_id` |

**Omit (retired 37f / 039):** `general_zones` — General bucket not restored by 42a.
...
- **`scopes`** — all root `site_zone` rows for site (`parent_zone_id IS NULL` and `root_item_id IS NOT NULL`) ordered by `sort_order`; join catalog `item.name` as `root_item_name`; nest child `site_zone` rows by `parent_zone_id` (single-table tree walk).
...
| `patch` | manifest-narrowed `profile`, `customer_party`, `property_owner_party`, `contacts`, **`scopes`** | Portfolio scalars accept id or `null`; collections replace-array |
...
`site_contact` cascade on site delete (DDL). **`site_zone`:** replace-array on PATCH; omit unreferenced rows (hard delete); **409** when referenced by `estimate_line` (via allocation), `job_line`, or `site_asset`; **root delete additionally blocked** while any child row exists ([task 42a](../tasks/42a-site-zone-tree-unification.md) D2). **App pre-check** required for `child_site` — DB `ON DELETE SET NULL` would silently unparent children; product rule is **block** until children are deleted or reparented (`parent_site` UI deferred).
...
- **Sites are logical places** — no postal address columns; addresses live on `party_address` (wave 2); in-building scope on unified **`site_zone`** tree ([`01-site-as-built.md`](../planning/01-site-as-built.md), [42a](../tasks/42a-site-zone-tree-unification.md)).
