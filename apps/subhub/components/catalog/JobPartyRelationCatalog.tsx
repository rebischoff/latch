...
type JobPartyRelationCatalogProps = {
  manifest: Manifest;
};
...
const DisplayNameCell = ({
  ...
  <Controller<RelationFormValues>
    ...
    render={({ field: { value, onChange, onBlur } }) => {
      ...
      return writable ? (
        <Input value={text} onChange={onChange} onBlur={onBlur} disabled={disabled} />
      ) : (
...
const RELATION_COLUMNS: FieldArrayTableColumn<RelationFormValues, "rows">[] = [
  {
    key: "display_name",
    title: "Display name",
...
export const JobPartyRelationCatalog = ({
  manifest: initialManifest,
}: JobPartyRelationCatalogProps) => {
  ...
  const saveRows = async (rows: RelationFormRow[]) => {
    await patchSurfaceList("job_party_relation_table", {
      rows: rows.map((row) => ({
        ...(row.id ? { id: row.id } : {}),
  ...
  if (error) {
    return (
      <Typography.Text type="danger">Unable to load party relations.</Typography.Text>
    );
  }
