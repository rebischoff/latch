...
type ParticipatesRow = CategoryDetailFormValues["spec_participation"]["participates"][number];
...
const isAssignedHere = (
  ...
  categoryId: string,
): boolean => participation?.assign_category_id === categoryId;
...
const isRowEditable = (
  ...
  if (!participation?.assign_category_id) {
    return isRoot;
  }
  return participation.assign_category_id === categoryId;
};
...
export const CategorySpecDefinitionsField = ({
  ...
  const { control, watch, setValue } = useFormContext<CategoryDetailFormValues>();
...
const SpecOptionsReadOnly = ({ index }: { index: number }) => {
  const { watch } = useFormContext<CategoryDetailFormValues>();
