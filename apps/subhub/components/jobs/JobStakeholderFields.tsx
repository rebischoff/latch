...
export type JobStakeholderFormRow = {
  display_name: string;
  kind?: string;
  party_id: string;
  relation_id: string;
...
export type JobStakeholdersFormValues = {
  stakeholders: JobStakeholderFormRow[];
};

type JobStakeholderFieldsProps = {
  manifest: Manifest;
};
...
export const validateJobStakeholderDuplicates = <T extends FieldValues>(
  stakeholders: JobStakeholderFormRow[],
  ...
  stakeholders.forEach((row, index) => {
    ...
    if (priorIndex !== undefined) {
      const message = "This party already has this relation on the job";
...
const PartyCell = ({
  ...
  const { setValue, watch } = useFormContext<JobStakeholdersFormValues>();
  const displayName = watch(`stakeholders.${index}.display_name`);

  return (
    <Controller<JobStakeholdersFormValues>
      name={`stakeholders.${index}.party_id` as FieldPath<JobStakeholdersFormValues>}
...
const RelationCell = ({
  ...
  const { watch } = useFormContext<JobStakeholdersFormValues>();
  const relationLabel = watch(`stakeholders.${index}.relation_label`);

  return (
    <Controller<JobStakeholdersFormValues>
      name={`stakeholders.${index}.relation_id` as FieldPath<JobStakeholdersFormValues>}
      render={({ field, fieldState }) =>
        writable ? (
          <Select
            showSearch
...
export const JobStakeholderFields = ({ manifest }: JobStakeholderFieldsProps) => {
  const { watch } = useFormContext<JobStakeholdersFormValues>();
  ...
  const columns = useMemo<
    FieldArrayTableColumn<JobStakeholdersFormValues, "stakeholders">[]
  >(
    () => [
      {
        key: "party",
  ...
  return (
    <FieldControl manifest={manifest} field="stakeholders">
      <FormSection title="Stakeholders">
        {emptyState}
        <FieldArrayTable<JobStakeholdersFormValues, "stakeholders">
          field="stakeholders"
          name="stakeholders"
          columns={columns}
          createRow={() => ({
