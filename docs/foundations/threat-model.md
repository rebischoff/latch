...
- Out: network / infra threats (TLS, DDoS, secret leakage), browser-side vulnerabilities (XSS, CSRF — handled by Next.js + standard practice), social engineering.
...
| **T3** | Stale manifest exploit | Tab open while admin revoked role; user submits form | Server re-resolves manifest on every mutation | Revoke role mid-session ? next write returns 403 |
...
| **T5** | RLS bypass via privileged DB role | Code path runs as superuser or `SECURITY DEFINER` function | App connects with non-superuser `latch_app` role; CI asserts no superuser at runtime | Connection test: `SELECT current_user` ? admin |
...
| **T7** | Pending change tampering | Submitter mutates own pending after decision; reviewer flips decision | Pending row immutable post-decision; state machine enforced in DAL | Try `PATCH /pending/:id` after acceptance ? 409 |
...
| **T10** | Approval bypass via direct DAL | A new code path writes a Field that should require approval | DAL refuses write to Fields marked `requires_approval` unless context flag set by approval applier | Unit test: direct write to gated Field ? throws |
...
| **T12** | Session var leak across requests | `SET LOCAL` forgotten; previous request's actor/company carries over | Every DB call wrapped in transaction that begins with `SET LOCAL`; middleware asserts | Integration test rapidly alternates two actors ? audit shows correct actor each time |
