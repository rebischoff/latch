...
How the DAL handles **bulk update** and **bulk delete** (soft delete in v1) under Field- and row-level permissions.
...
- Partial is what trades-CRM users actually want (S2 in [`../use-cases.md`](../use-cases.md)): "reassign these 20, tell me which I couldn't."
...
2. **Load candidate rows** with the same row filters the single-record DAL would apply. Rows not visible ? reported as `not_found` (existence-hiding) or `forbidden_row` per global option.
...
6. **Audit:** one row per changed entity (action `update` / `soft_delete`), plus one optional `bulk_summary` row with `request_id`, counts, mode.
...
- **T16** Soft-delete audit gap: bulk soft-delete N rows ? N audit rows present.
