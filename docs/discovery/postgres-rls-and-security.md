...
> **Status (2026-05-27): DEFERRED post-v1.** v1 enforcement is **DAL-only**. See [`../scope.md`](../scope.md). This doc remains the design target for the Phase 4 hardening spike — do not implement in v1.
...
| Column privileges | `GRANT SELECT (col) ON ...` — static, poor fit for dynamic Field sets |

## Spike plan (Phase 4 — post-v1)

1. **Spike A — Company/row RLS:** policy keyed on `app.principal_id` / `app.company_id`.
2. **Spike B — Field mask view:** base table + view exposing subset of columns; role connects to view only.
3. **Spike C — Audit trigger hardening:** trigger writes to audit table as a role that can only INSERT.
4. **Spike D — Pending table RLS:** submitter vs reviewer policies.
...
- OPA / Cedar for policy DSL — too heavy for our use case.
- Supabase / PostgREST patterns — reference only; opposite architectural direction.
